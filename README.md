...
> **Latest changes (2026-06-08):** The **Mobile Applications** module (toggleable from Settings → Modules; route `/mobile-apps`) tracks your own apps' **App Store + Google Play reviews and ratings** through the **official store APIs only** — the Google Play **Android Publisher API** (`reviews.list`, service account) and the Apple **App Store Connect API** (`customerReviews`, ES256 JWT). It is strictly **view-only**: it reads reviews and ratings and never replies. Credentials are loaded from a `secrets.env` file (service-account JSON path + `.p8` key path), never from the database and never exposed to the browser; see `secrets.env.example`. The per-app detail shows a per-store rating masthead, a star distribution, a review-based rating-trend chart, and a **paginated, filterable, searchable reviews feed**. Sync is **automatic on page load** (stale-while-revalidate) with SSE live updates and a short de-dupe window so the APIs aren't hammered. **On-demand agent sentiment digests** ("AI sentiment") reuse the same local `openclaw agent` dispatch as Processes simulation, so the `openclaw` CLI must be on the server PATH. Ratings/averages are computed from the reviews actually fetched, since the publisher APIs expose no store-wide aggregate. Dependencies: `@googleapis/androidpublisher`, `google-auth-library`, `jose`, `zod`, `p-limit` — run `npm install`.
