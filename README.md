...
> **Latest changes (2026-06-05):** New **Mobile Applications** module (toggleable from Settings → Modules; route `/mobile-apps`). Track your own apps' **App Store + Google Play reviews and ratings** in real time — add an app by pasting its App Store and/or Play URL/ID, and the per-app detail shows a ratings-over-time chart, star histogram, and a filterable reviews feed. Data sources (v1) are **free public sources** behind a provider interface (so official APIs can be swapped in later): Apple customer-reviews RSS + iTunes Lookup, and the `google-play-scraper` package — no credentials needed. Sync is **real-time on page load** (stale-while-revalidate) plus a "Refresh now" button and SSE live updates, with a short de-dupe window so the stores aren't hammered. **On-demand agent sentiment digests** ("Generate digest now") reuse the same local `openclaw agent` dispatch as Processes simulation, so the `openclaw` CLI must be on the server PATH. **Threshold alerts** (e.g. avg rating drops below X) are evaluated on each sync. _v1 limitations:_ alerts only fire when a sync runs (a page view/refresh) — there is no background scheduler yet; alert notifications surface in the **Dashboard activity feed** (`activity_logs`) only — Telegram/Slack delivery is deferred (no reusable server-side notification helper yet); the alerts list/CRUD route is not workspace-scoped (fine for the current single-workspace deployment); Apple RSS is capped at roughly the most recent ~500 reviews per country, and Play scraping is unofficial so it can break if Google changes their markup. New dependency: `google-play-scraper` — run `npm install`.
