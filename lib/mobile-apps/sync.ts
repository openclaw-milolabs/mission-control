...
import {
    loadMobileReviewsConfig,
    type GoogleConfig,
} from "@/lib/mobile-apps/config";
import { summarizeReviews } from "@/lib/mobile-apps/metrics";
import { getProvider } from "@/lib/mobile-apps/providers";
import {
    fetchAppleTerritoryRatings,
    type TerritoryRating,
} from "@/lib/mobile-apps/providers/app-store-ratings";
import {
    downloadReportFile,
    listReportFiles,
    parseRatingsCsv,
...
import {
    ratingSourceCopy,
    type RatingSource,
} from "@/lib/mobile-apps/rating-source";
...
async function persistMultiReports(
    ...
    for (const r of records) {
        ...
        const canonical = JSON.stringify(
            Object.keys(r.dimensions)
                .sort()
                .map((k) => [k, r.dimensions[k]]),
        );
...
async function cachedReportFile(
    sql: Sql,
    listingId: string,
    objectPath: string,
): Promise<CachedReportFile | null> {
...
async function listFilesOrWarning(
    ...
    try {
        ...
        return {
            files,
            warning: files.length
                ? null
                : reportNotFoundWarning(label, cfg, appIdentifier, dimensions),
        };
    } catch (err) {
        return {
            files: [],
            warning: `${label} report: ${reportErrMessage(err)}`,
        };
    }
}
...
async function syncSingleDimensionReportFiles(
    ...
    const { files, warning } = await listFilesOrWarning(
        cfg,
        report,
        appIdentifier,
        dimensions,
    ...
    for (const file of files) {
        ...
        try {
            ...
            await markReportFile(
                sql,
                listingId,
                file,
                records.length > 0 ? "parsed" : "empty",
                records.length,
                null,
            );
            filesDownloaded++;
            ...
            await markReportFile(sql, listingId, file, "failed", 0, msg).catch(
                () => null,
            );
...
async function loadGoogleRatingsFromDb(
    sql: Sql,
    listingId: string,
): Promise<{ territories: TerritoryRating[]; asOf: string | null }> {
    const rows = (await sql`
...
async function syncListing(
    ...
    const recentlySynced =
        ...
        Date.now() - new Date(listing.last_synced_at).getTime() < opts.dedupeMs;
    if (!opts.force && recentlySynced) {
        return {
            ...
            ratingSourceHelperText:
                "This listing was skipped because it was synced recently.",
        };
    }
  ...
  `) as unknown as Array<{ id: string }>;
    ...
    try {
      ...
      `;
            if ((res as unknown as Array<{ inserted: boolean }>)[0]?.inserted)
                inserted += 1;
        }
    ...
    `;
        ...
        if (store === "apple") {
            const territories = [
                ...reviews.map((r) => r.country ?? ""),
                listing.country,
            ];
            const mode = cfg.apple.fullStorefrontScan;
            const fullScan =
                mode === "always" || (mode === "forced" && opts.force);
            officialRatings = await fetchAppleTerritoryRatings(
                appIdentifier,
                territories,
                {
                    fullScan,
            ...
            const primary =
                officialRatings.find(
                    (t) => t.territory === toAlpha2(listing.country),
                ) ??
                officialRatings[0] ??
                null;
            ...
            if (cfg.google.reportsBucket) {
                ...
                const ratingReport = await loadGoogleRatingsFromDb(
                    sql,
                    listing.id,
                );
    ...
    `;
        ...
        const ratingCaptured =
            currentRating != null || officialRatings.length > 0;
        const sourceCopy = ratingSourceCopy({
            store,
            source: ratingSource,
            asOf: ratingAsOf,
        });
      ...
      `.catch(() => null);
        ...
        return {
            ...
            ratingFreshnessLabel: null,
            ratingSourceHelperText:
                "The sync failed before a rating source could be refreshed.",
        };
    }
}
...
export async function syncApp(
  ...
  `) as unknown as ListingRow[];
    ...
    const shouldRefreshReports = refreshReports ?? force ?? false;
        ...
        listings.map((l) =>
            limit(() =>
                syncListing(sql, l, {
                    force,
                    dedupeMs,
                    refreshReports: shouldRefreshReports,
                }),
            ),
        ),
    );
    ...
    await sql`select pg_notify('mobile_apps_change', ${JSON.stringify({ appId })})`.catch(
        () => null,
    );
    return results;
}
